 ...
 * It sets up a mock-version of highscoreService, and then uses this mock to test
...
public class HighscoreRestControllerTest {
    ...
    @Autowired
    private MockMvc mockMvc; // This guy is used to simulate HTTP interactions. It's a mock, so it's not a real server.

    @MockBean
    private HighscoreService highscoreService;

    @Autowired
    private ObjectMapper objectMapper; // Instantiating a new field variable of type ObjectMapper with the @Autowired annotation.
    ...
    public void testGetAllHighscores() throws Exception {
        ...
        mockMvc.perform(get("/highscores")) // Perform a GET request to the /highscores endpoint.
                ...
                .andExpect(content().json(objectMapper.writeValueAsString(highscores))); // Check that the response in JSON format is equal to the highscores list written as a JSON string.
    }
